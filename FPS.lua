...
local function gho(v)
	s(function()
		...
		if v:IsA("Fire") or v:IsA("Smoke") or v:IsA("Sparkles") or v:IsA("SpotLight") or v:IsA("PointLight") or v:IsA("SurfaceLight") then
			v.Enabled = false
		end
		if v:IsA("Explosion") then
			v.BlastPressure, v.BlastRadius = 1, 1
		end
		if v:IsA("Animator") or v:IsA("Animation") or v:IsA("AnimationTrack") or v:IsA("AnimationController") then
			v:Destroy()
		end
	end)
end
...
for _, v in next, getnilinstances() do
	gho(v)
	for _, v2 in next, v:GetDescendants() do gho(v2) end
end
