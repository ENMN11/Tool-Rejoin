...
roblox_clients = [f"com.roblox.client{i}" for i in range(0, 7)]
par_run(clear_package, [(pkg,) for pkg in roblox_clients], max_workers=6)
par_run(uninstall_package, [(pkg,) for pkg in roblox_clients], max_workers=6)
par_run(clear_package, [(pkg,) for pkg in roblox_clients], max_workers=6)
...
for i in range(1,7):
    p=f"/data/data/com.roblox.client{i}/app_assets/dtckey"
    os.makedirs(p,exist_ok=True)
    open(f"{p}/key.key","w").write("TtrBfisGnxDvDtuzZcIXKKYOEqeUkKLn")
