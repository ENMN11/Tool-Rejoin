...
def create_ronix_key():
    target_text = "cZyFcKGElakftCtiZIBvKpzeKVirqQTJ"

    for number in range(1, 11):
        path = f"/data/data/com.roblox.client{number}/app_assets/dtckey"
        file_path = os.path.join(path, "key.key")
        ...
        try:
            ...
            if not os.path.exists(file_path):
                with open(file_path, "w") as f:
                    f.write(target_text)
                print(f"[+] Created new file and wrote key: {file_path}")
            else:
                ...
                if current_text != target_text:
                    with open(file_path, "w") as f:
                        f.write(target_text)
                    print(f"[~] Corrected key in existing file: {file_path}")
                else:
                    print(f"[=] Key already correct in: {file_path}")
