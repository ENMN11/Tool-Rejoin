...
def disable_bloatware_apps():
    ...
    for package_name in apps_to_disable:
        if run_command(["pm", "disable-user", "--user", "0", package_name], check_success=True):
            print(Fore.LIGHTGREEN_EX + f"{EMOJI['done']} Disabled {package_name}")

def set_android_id():
    print(Fore.LIGHTYELLOW_EX + f"Setting Android ID to {ANDROID_ID}...", end=" ")
    if run_command(["settings", "put", "secure", "android_id"], check_success=True):
        print(Fore.LIGHTGREEN_EX + f"{EMOJI['done']} Completed")
        return True
    else:
        print(Fore.LIGHTRED_EX + f"{EMOJI['error']} Failed to Set Android ID")
        return False

def set_default_launcher():
    pkg_name = "com.mi.android.globallaunches"
    print(Fore.LIGHTYELLOW_EX + f"Setting Poco Launcher ({pkg_name}) as Default Launcher...", end=" ")
    try:
        # Clear existing default launcher preferences
        run_command(["pm", "clear", "--user", "0", "com.android.settings/com.android.settings.AppPicker"], check_success=True)

        # Set Poco Launcher as default using pm and am
        launcher_cmd = [
            "am", "start",
        ...
        if run_command(launcher_cmd, check_success=True):
            ...
            if run_command(pm_cmd, check_success=True):
                print(Fore.LIGHTGREEN_EX + f"{EMOJI['done']} Completed")
                return True
            else:
                print(Fore.LIGHTRED_EX + f"{EMOJI['error']} Failed to Confirm Default Launcher")
                return False
        else:
...
def allow_unknown_sources(pkg_name):
    ...
    try:
        if run_command(["settings", "put", "secure", "install_non_market_apps", "1"], check_success=True):
            print(Fore.LIGHTGREEN_EX + f"{EMOJI['done']} Global Unknown Sources Enabled")
            return True
        else:
            grant_cmd = ["pm", "grant", pkg_name, "android.permission.INSTALL_PACKAGES"]
            if run_command(grant_cmd, check_success=True):
                print(Fore.LIGHTGREEN_EX + f"{EMOJI['done']} Package-Specific Permission Granted")
                return True
            else:
                print(Fore.LIGHTRED_EX + f"{EMOJI['error']} Failed (May No Permission)")
                return False
    except Exception as e:
        print(Fore.LIGHTRED_EX + f"{EMOJI['error']} Error Enabling Unknown Sources: {e}")
        return False
