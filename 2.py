...
class ExecutorManager:
    ...
    def detect_executors():
        ...
        for executor_name, base_path in executors.items():
            workspace_path_lower = os.path.join(base_path, "workspace")
            workspace_path_upper = os.path.join(base_path, "Workspace")

            if os.path.exists(workspace_path_lower) or os.path.exists(workspace_path_upper):
    ...
    def write_lua_script(detected_executors):
        # This function is now empty as per the request to remove all auto-execution logic.
        pass
...
def main():
    ...
    if not globals().get("command_8_configured", False):
        globals()["check_exec_enable"] = "1"
        globals()["lua_script_template"] = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/ENMN11/NexusHideout/refs/heads/main/Ayaya.luau"))()'
