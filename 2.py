...
# Fix: Use psutil directly to avoid variable name conflict
...
executors = {
    ...
    # ... (Giữ nguyên danh sách executors của bạn, tôi rút gọn hiển thị để code không quá dài)
    "Delta": "/storage/emulated/0/Delta/",
    "KRNL": "/storage/emulated/0/krnl/",
    "Trigon": "/storage/emulated/0/Trigon/",
    "Evon": "/storage/emulated/0/Evon/",
}
# Tự động thêm các clone khác vào executors nếu cần, logic workspace giữ nguyên
...
class FileManager:
    ...
    # FIX: Added missing method 'xuat' used in monitor_presence
    @staticmethod
    def xuat(file_path):
        """
        Đọc giá trị cookie .ROBLOSECURITY từ file database SQLite.
        Sử dụng cơ chế copy file tạm để tránh lỗi 'Database is locked' khi game đang chạy.
        """
        try:
            if not os.path.exists(file_path):
                return None
            
            # Copy file ra thư mục tạm để đọc an toàn
            temp_path = file_path + ".temp_read"
            try:
                shutil.copy2(file_path, temp_path)
            except IOError:
                # Nếu không copy được (do permission), thử đọc trực tiếp (có rủi ro lock)
                temp_path = file_path
            ...
            # Utilities.log_error(f"Error extracting cookie from DB {file_path}: {e}")
            return None
    ...
    def download_file(url, destination, binary=False):
        try:
            ...
            if response.status_code == 200:
                ...
                # Handle text encoding if not binary
...
class RobloxManager:
    ...
    def get_roblox_packages():
        ...
        try:
            package_prefix = globals().get("package_prefix", "com.roblox")
            # Use a safer way to split lines
    ...
    def kill_roblox_processes():
        ...
        killed_any = False
        for package_name in packages:
            # Check if this specific package is running
        ...
        if setup_type == "1":
            try:
                ...
                if not globals()["accounts"]:
                    print("\033[1;31m[ Shouko.dev ] - Setup ran, but no User IDs were found. Cannot start Auto Rejoin.\033[0m")
                ...
                if not server_links:
                    print("\033[1;31m[ Shouko.dev ] - No game ID or server link set up. Please run option 2 first.\033[0m")
                    input("\033[1;32mPress Enter to return...\033[0m")
                    continue
