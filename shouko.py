...
version = "1.0.0 | Created By Neyoshi And Improved By Nexus"
...
        try:
            # Sửa lỗi permission khi đọc file hệ thống trên Android
...
    # === FIX START: PERMISSION AND OWNER FIX ===
    @staticmethod
    def fix_permissions(path, package_name):
        """Thay đổi chủ sở hữu file thành UID của package và cấp quyền đọc/ghi"""
        try:
            # Lấy UID:GID của thư mục gốc data package
...
                cmd_chmod = f"chmod 660 {path}" # Chủ sở hữu và nhóm có quyền đọc/ghi
                subprocess.run(cmd_chmod, shell=True)
                # Android 10+ có thể cần restorecon cho SELinux
...
        """Cập nhật UserId trong appStorage.json để khớp với cookie"""
        try:
            with open(file_path, 'r') as f:
                data = json.load(f)
            
            # Cập nhật UserId chính
            data['UserId'] = str(user_id)
            
            # Nếu có AppConfiguration, cập nhật lại UserID trong đó nếu cần (tùy chọn, nhưng UserId root là quan trọng nhất)
            # Ghi lại file
...
                # 1. Chuẩn bị appStorage.json: Cập nhật UserId trước khi copy
...
                
                # 2. Chuẩn bị Cookies: Copy và Update DB
...
                # 3. FIX QUAN TRỌNG: Sửa quyền sở hữu file (Android 10+ Root)
...
                # 4. LAUNCH APP (Tương tự Option 8)
...
                time.sleep(5) # Đợi app load một chút trước khi làm package tiếp theo
...
            cursor = conn.cursor()
            # Xử lý an toàn hơn: Update nếu có, nếu không thì Insert (nhưng template chuẩn nên có sẵn)
...
                # Fallback nếu template DB trống rỗng hoặc khác cấu trúc (ít xảy ra với template chuẩn)
...
    @staticmethod
    def check_executor_status(package_name, continuous=True, max_wait_time=95):
...
    @staticmethod
    def check_executor_and_rejoin(package_name, server_link, next_package_event, stop_event):
        user_id = globals()["_user_"][package_name]
        detected_executors = ExecutorManager.detect_executors()

        if not detected_executors:
            error_msg = f"NO EXECUTOR DETECTED FOR {package_name}! EXITING TOOL."
            print(f"\033[1;41m[ Shouko.dev ] - {error_msg}\033[0m")
...
        
        with status_lock:
            globals()["package_statuses"][package_name]["Status"] = "\033[1;33mWaiting for Executor...\033[0m"
            UIManager.update_status_table()
...
            
            for workspace in globals()["workspace_paths"]:
                check_path = os.path.join(workspace, f"{user_id}.main")
...
                if is_first_load:
                    is_first_load = False
                    msg = "\033[1;32mExecutor Loaded Successfully\033[0m"
                    next_package_event.set()
                
                with status_lock:
                    if globals()["package_statuses"][package_name]["Status"] != msg:
                        globals()["package_statuses"][package_name]["Status"] = msg
                        UIManager.update_status_table()
                
                time.sleep(20) 
                
...
                remaining_time = 95 - elapsed_time
                
                if elapsed_time > 95:
                    print(f"\033[1;31m[ Shouko.dev ] - {package_name}: Executor crash detected (No signal for 95s). Restarting...\033[0m")
                    
                    with status_lock:
                        globals()["package_statuses"][package_name]["Status"] = "\033[1;31mCrash Detected -> Restarting...\033[0m"
                        UIManager.update_status_table()
                    
                    RobloxManager.kill_roblox_process(package_name)
                    RobloxManager.delete_cache_for_package(package_name)
...
                    ExecutorManager.reset_executor_file(package_name)
                    
                    print(f"\033[1;33m[ Shouko.dev ] - Re-launching {package_name}...\033[0m")
...
                    
                else:
                    status_msg = f"\033[1;33mNo signal: {int(elapsed_time)}s/95s\033[0m"
                    if not is_first_load:
                        status_msg = f"\033[1;33mExecutor Idle/Crash? {int(elapsed_time)}s/95s\033[0m"
                        
                    with status_lock:
                         globals()["package_statuses"][package_name]["Status"] = status_msg
                         UIManager.update_status_table()
                    
                    time.sleep(10)
...
                    detected_executors = ExecutorManager.detect_executors()
                    
                    if package_name in globals()["thread_stop_events"]:
                         globals()["thread_stop_events"][package_name].set()
                    new_stop_event = threading.Event()
                    globals()["thread_stop_events"][package_name] = new_stop_event
...
                            args=(package_name, server_link, next_package_event, new_stop_event),
                            daemon=True
                        ).start()
                    else:
                         threading.Thread(
...
        for package_name in packages:
            try:
                # 1. Tắt app trước để tránh lỗi file đang dùng
                RobloxManager.kill_roblox_process(package_name)
                
                # 2. Định nghĩa đường dẫn file cần xóa
                # File Cookies (thường không có đuôi .db nhưng là file sqlite)
                cookie_path = f"/data/data/{package_name}/app_webview/Default/Cookies"
                cookie_journal = f"/data/data/{package_name}/app_webview/Default/Cookies-journal" # Xóa luôn file journal nếu có
                # File appStorage.json
                app_storage_path = f"/data/data/{package_name}/files/appData/LocalStorage/appStorage.json"
                
                # 3. Thực hiện lệnh xóa
                cmd_delete = f"rm -f \"{cookie_path}\" \"{cookie_journal}\" \"{app_storage_path}\""
...
                         # Sử dụng monkey để launch main activity một cách universal
